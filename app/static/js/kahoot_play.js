...
(function () {
  ...
  function renderResult(r) {
    ...
    if (!r.answered) {
      ...
      if (typeof confetti === "function") {
        confetti({ particleCount: 80, spread: 70, origin: { x: 0.5, y: 0.6 }, ticks: 150 });
      }
    } else {
      els.result.classList.add("bad");
      els.resultTitle.textContent = "Helaas...";
      els.resultPoints.textContent = "+0";
      playSound(els.incorrectSound);
    }
  ...
  function renderFinished(data) {
    ...
    show("finished");
    if (data.rank <= 3 && typeof confetti === "function") {
      confetti({ particleCount: 150, spread: 90, origin: { x: 0.5, y: 0.5 }, ticks: 250 });
    }
  }
